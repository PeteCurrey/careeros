      ...
      <div className="p-6 sm:p-8 bg-black/30 backdrop-blur-sm border-b border-[var(--color-border-default)] flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <span className="text-[10px] font-mono uppercase tracking-widest text-[#6BB8FF] font-semibold flex items-center gap-1.5">
            <Layers className="w-3.5 h-3.5 text-[#2F8FFF]" />
        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="border-b border-[var(--color-border-default)] text-[10px] font-mono uppercase text-[var(--color-taupe-300)]">
              ...
              <th className="pb-3 px-3 font-semibold text-white">AI Involvement</th>
              <th className="pb-3 px-3 font-semibold text-purple-300">Human Review &amp; Agency</th>
              <th className="pb-3 px-3 font-semibold text-amber-300">Impact Tier</th>
              <th className="pb-3 pl-3 font-semibold text-emerald-300">Safeguards</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--color-border-subtle)]">
            {filteredItems.map((item) => (
              <tr key={item.capability} className="hover:bg-white/5 transition-colors">
                <td className="py-4 pr-4 align-top max-w-xs space-y-1">
                  <span className="font-semibold text-white block text-xs">{item.capability}</span>
                ...
                <td className="py-4 px-3 align-top text-[11px] text-white max-w-xs">
                  {item.humanReview}
                </td>
                ...
                <td className="py-4 pl-3 align-top text-[11px] text-[var(--color-text-secondary)] space-y-1">
                  {item.safeguards.map((sg) => (
                    <div key={sg} className="flex items-center gap-1.5 text-white/80">
          ...
          >
            ...
            <div className="space-y-1.5 pt-2 border-t border-[var(--color-border-subtle)]">
              <div>
                ...
                <span className="text-white">{item.aiRole}</span>
              </div>
              <div>
                <span className="text-[10px] font-mono text-[var(--color-taupe-300)] uppercase block">Human Oversight:</span>
                <span className="text-purple-300">{item.humanReview}</span>
              </div>
            </div>

            <div className="pt-2 border-t border-[var(--color-border-subtle)] space-y-1">
              <span className="text-[10px] font-mono text-emerald-400 uppercase block">Safeguards:</span>
              <div className="flex flex-wrap gap-1.5">
                {item.safeguards.map((sg) => (
                  <span key={sg} className="px-2 py-0.5 rounded bg-white/5 border border-white/10 text-[10px] text-white">
                    {sg}
                  </span>
                ))}
              </div>
