...
export function DocumentAccessRequestModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[color-mix(in_srgb,var(--color-surface-dark-deep)_82%,transparent)] backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg rounded-lg bg-[var(--background-dark-deep)] border border-[var(--color-border-default)] shadow-2xl p-6 sm:p-8 space-y-6 text-[var(--color-text-primary)]">
        ...
        {success ? (
          <div className="space-y-4 text-center py-4">
            <div className="w-12 h-12 rounded-full bg-[var(--color-success)]/10 border border-[var(--color-success)]/30 text-[var(--color-success)] flex items-center justify-center mx-auto">
            ...
            <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed max-w-md mx-auto">
              Thank you, {name}. Your request for <span className="font-semibold text-[var(--color-text-primary)]">{documentName}</span> has been routed to the CareerOS Trust & Assurance Office. Our team will verify your institutional credentials and send a mutual NDA link to <span className="font-mono text-[var(--color-text-primary)]">{email}</span> within 1 business day.
            </p>
            <div className="pt-4">
              <button
                type="button"
                onClick={onClose}
                className="px-5 py-2 text-xs font-semibold rounded-[var(--radius-button)] bg-[var(--color-surface-raised)] text-neutral-900 hover:bg-neutral-100 transition-colors"
              >
                Close Window
              </button>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <div className="inline-flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-widest text-[var(--color-brand-300)]">
            ...
            <div className="p-3 rounded-md bg-[var(--color-surface-raised)] border border-[var(--color-border-subtle)] space-y-1">
              ...
              <p className="text-xs font-bold text-[var(--color-text-primary)]">{documentName}</p>
            </div>

            {errorMessage && (
              <div className="p-3 rounded-md bg-[var(--color-danger)]/10 border border-[var(--color-danger)]/30 text-[var(--color-danger)] text-xs flex items-center gap-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-sm bg-[var(--color-surface-sunken)] border border-[var(--color-border-default)] focus:border-[var(--accent-blue)] focus:outline-hidden text-xs text-[var(--color-text-primary)]"
                  />
                </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 rounded-sm bg-[var(--color-surface-sunken)] border border-[var(--color-border-default)] focus:border-[var(--accent-blue)] focus:outline-hidden text-xs text-[var(--color-text-primary)] resize-none"
                />
              </div>
              ...
              <div className="pt-2 flex items-center justify-between">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-[var(--radius-button)] bg-[var(--accent-blue)] hover:bg-[var(--accent-blue)]/90 text-[var(--color-text-primary)] font-semibold text-xs transition-colors disabled:opacity-50"
                >
                  {submitting ? (
                    <>
