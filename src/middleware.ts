...
// Cookie names come from their own dependency-free module. Importing them from
// @/lib/admin/session pulled node:crypto into the Edge bundle, which Next warns
// about on every boot.
import {
...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith("/app") || pathname.startsWith("/api/app")) {
    // A Supabase SSR session is identified by cookie name only — its contents
    // are verified server-side by supabase.auth.getUser() in the access guard,
    // which is the authority. Middleware is a cheap first gate, not the check.
    const hasSupabaseSession = request.cookies
      .getAll()
      .some((c) => c.name.includes("-auth-token") || c.name.startsWith("sb-"));

    // The fallback cookie, by contrast, previously passed this gate on the
    // strength of its NAME alone, so a forged cookie reached the application.
    // It must now carry a signature this server issued.
    const fallbackSession = await verifyUserSession(
      request.cookies.get(USER_SESSION_COOKIE_NAME)?.value,
    );

    const hasAuthCookie = hasSupabaseSession || fallbackSession !== null;
